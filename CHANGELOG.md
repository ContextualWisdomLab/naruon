## [Unreleased]
- 유틸리티 도구 2종(`hash_generator`, `email_phone_masker`)을 추가해 텍스트의 SHA-256 해시를 생성하고 ASCII 이메일 주소와 일부 한국 전화번호 형식을 단순 마스킹합니다. 마스킹은 완전한 개인정보 비식별화를 보장하지 않으며, 두 도구 모두 입력 길이를 제한합니다.
