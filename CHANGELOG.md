## [Unreleased]
- URL 인코딩(`url_encoder`), URL 디코딩(`url_decoder`), JSON 포매터(`json_formatter`) 3개의 새로운 유틸리티 도구를 `backend/api/tools.py`에 추가했습니다. JSON 포매터는 표준 JSON 숫자 상수만 허용하고 다국어 문자를 보존합니다.
