## [Unreleased]
- URL 인코딩(`url_encoder`), URL 디코딩(`url_decoder`), JSON 포매터(`json_formatter`) 3개의 새로운 유틸리티 도구를 `backend/api/tools.py`에 추가하여 텍스트 데이터의 변환 및 가독성을 높였습니다. JSON 포매터는 유효하지 않은 입력과 비표준 숫자 및 유니코드 출력을 안전하게 처리하도록 검증되었습니다.
