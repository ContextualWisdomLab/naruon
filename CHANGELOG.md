## [Unreleased]
- `text_reverser` 유틸리티를 canonical hash/JSON 도구 뒤에 추가하고 공통 100,000자 입력 제한을 적용했습니다.
- `backend/api/tools.py`에 `hash_generator`(텍스트 해시화) 및 `json_formatter`(압축된 유효한 JSON 문자열 정렬) 도구를 추가했습니다.
