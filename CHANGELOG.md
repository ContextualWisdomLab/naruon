...
- **링크 추출기 (Link Extractor)** 도구를 `backend/api/tools.py`에 새롭게 추가하여 AI 허브 등에서 활용 가능한 텍스트 처리 능력을 강화하였습니다. `Link Extractor`는 텍스트에서 URL 목록과 그 개수를 추출합니다.
- `backend/tests/test_tools_api.py`에 새로 추가된 도구에 대한 자동화 테스트 코드를 작성하였고 엣지 케이스까지 모두 포괄하여 `api.tools` 모듈의 100% 테스트 커버리지를 보장합니다.
