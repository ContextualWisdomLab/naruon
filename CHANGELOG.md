## [Unreleased]
- 사용자 편의를 위한 6종 유틸리티 도구(URL 인코더, URL 디코더, JSON 포매터, HTML 이스케이프, HTML 언이스케이프, 해시 생성기)를 추가했습니다.
- 유틸리티 입력은 100,000자로 제한하며, JSON 포매터는 숫자 원문을 보존하고 중복 키·비표준 숫자를 거부합니다. URL 디코더도 잘못된 UTF-8 이스케이프를 조용히 치환하지 않고 거부하며, 해시 생성기는 기본 SHA-256과 명시적 MD5/SHA-1/SHA-512 체크섬을 지원합니다.
- 도구 실행 콘솔이 더 이상 canned `test_value`를 제출하지 않고, 사용자가 편집한 파라미터를 타입에 맞춰 실행 요청에 전달합니다.
- 도구 콘솔은 성공한 도구의 실제 `result`를 표시하고, 새로고침 시 제거되거나 타입이 바뀐 파라미터를 이전 값으로 재사용하지 않습니다.
