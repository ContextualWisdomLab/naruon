## [Unreleased]
- 프로젝트 그래프 추출 경계를 바로잡았습니다. `keyword`는 명시적 결정론
  모드이며, `llm`과 `orchestrator`는 keyword 결과로 대체하지 않고
  실패를 그대로 알립니다. Naruon은 공급자·모델·그룹·pool·endpoint·자격증명을
  선택하지 않으며, immutable `contextual-orchestrator` 소비자 release가 없는
  동안 두 LLM selector는 fail closed 합니다. 아래의 과거 "지식그래프 추출기
  seam" 항목에 적힌 직접 공급자와 자동 keyword fallback 설명은 현재 계약이
  아닙니다.
