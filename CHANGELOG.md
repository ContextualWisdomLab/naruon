## [Unreleased]
- 사용자 편의를 위한 5종 유틸리티 도구(URL 인코더, URL 디코더, JSON 포매터, HTML 이스케이프, HTML 언이스케이프)를 추가했습니다.
- 유틸리티 입력은 100,000자로 제한하며, JSON 포매터는 숫자 원문을 보존하고 중복 키·비표준 숫자를 거부합니다. URL 디코더도 잘못된 UTF-8 이스케이프를 조용히 치환하지 않고 거부합니다.
