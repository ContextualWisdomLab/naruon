## [Unreleased]
- Starlette `TestClient`의 preferred transport를 `httpx2==2.13.0` / `httpcore2==2.13.0`으로 고정하고 core와 Noema-agent hash lock을 동일한 의존성 그래프로 동기화했습니다. deprecated `httpx` fallback 경고 억제도 제거해 transport 회귀가 warnings-as-errors 테스트에서 드러나도록 했습니다.
