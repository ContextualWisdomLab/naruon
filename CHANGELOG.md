## [Unreleased]
- NetworkGraph.tsx의 `useMemo`에서 불필요하게 `Array.from(...).slice(...)` 또는 `.map(...).filter(...).slice(...)`로 전체 리스트를 배열로 생성하고 O(N) 비용을 치르던 로직을 제거했습니다. 이를 고정 크기 조건에서 일찍 종료하는 `for...of` 루프 구조로 변경해, 데이터가 많을 때 발생하는 메모리 낭비와 렌더링 프레임 저하를 방지합니다.
