## [Unreleased]
- 프론트엔드의 `NetworkGraph.tsx` 컴포넌트에서, 대규모 Map 객체(`nodes`, `edges`)에 대해 `Array.from(...).slice(0, 5).map(...)`와 같이 체이닝되어 사용되던 배열 메서드를 `for...of` 루프와 `break`를 사용하는 O(1) 복잡도의 제한된 루프로 변경하여 일시적인 O(N) 배열 할당 및 메인 스레드 블로킹을 방지했습니다.
