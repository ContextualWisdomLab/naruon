...
### 변경됨 (Changed)
- `SearchLayout`과 `NetworkGraph` 컴포넌트에서 `disabled` 속성을 사용하는 버튼들을 `aria-disabled` 속성 기반으로 마이그레이션하여 스크린 리더와 키보드 접근성을 개선했습니다.
- 조건부로 비활성화되는 버튼의 접근성 툴팁(`title`, `aria-describedby`)이 포커스 상태에서도 정상적으로 동작하도록 수정했습니다.
