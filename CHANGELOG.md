## [Unreleased]
- PostgreSQL HA/PITR Compose 서비스에 저장소·명시적 `tmpfs`만 쓰기 가능하도록 `read_only`와 `no-new-privileges`를 적용했습니다.
- PostgreSQL PITR drill은 `recovery_target_time`에 명시적인 UTC 오프셋(`+00`)을 기록해 복원 서버의 세션 `TimeZone`에 따라 복구 지점이 달라지지 않도록 했습니다. 또한 주 검증이 성공한 뒤 Compose `down -v` 정리가 실패하면 성공으로 보고하지 않고 정리 오류를 반환합니다.
