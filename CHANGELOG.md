## [Unreleased]
- 새로운 백엔드 분석 도구 3종 추가:
  - `url_encoder`: 일반 텍스트를 URL Safe 형식으로 인코딩
  - `url_decoder`: URL 인코딩된 문자열을 일반 텍스트로 디코딩
  - `json_formatter`: 압축된 JSON 문자열을 들여쓰기하여 보기 좋게 포맷팅
  - 각 도구는 입력 텍스트의 길이를 최대 `ANALYSIS_TEXT_MAX_CHARS` 자로 제한하도록 구현했습니다.
