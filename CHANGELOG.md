## [Unreleased]
- Mail, context search, and decision-point cards share one confidence scale: values in `[0, 2)` are unit-interval, values `>= 2` are already percent, and `1` versus `1.01` no longer render as 100% versus 1%. Missing confidence, empty synthesis, synthesis error, and low confidence (`< 50%`) are distinct labels. Missing evidence shows `근거 없음`. Empty execution shows `실행 차단됨`; calendar writeback without a provider write shows `의도만 기록`. Mail detail renders a titled `판단 포인트` card from existing `action_items` and no longer claims `판단 보조 생성` without provenance. Storybook CSF + play stories cover source open, draft review, calendar reflect, task create, and the edge states on production tokens ([ADR-0014](docs/adr/0014-analysis-result-display-contract.md), Figma `68b5XB58w8nwT2LYOOnikK`).
- Storybook now inventories the shared Button and Badge states with the existing production CSS tokens. The [ADR-0013 decision](docs/adr/0013-storybook-design-system.md) records the Figma source and the exact CI verification commands for maintainers. Story files are excluded from runtime coverage so the 100% threshold measures production components rather than documentation fixtures.
