...
### 새로운 워크스페이스 도구 추가
- `backend/api/tools.py`에 새로운 도구(Tool) 2종을 추가했습니다:
  - **URL 추출기 (`url_extractor`)**: 텍스트 본문에서 URL 패턴을 인식하여 추출합니다.
  - **개인정보 마스킹 (`pii_anonymizer`)**: 텍스트 내 포함된 이메일 및 전화번호 패턴을 찾아 마스킹 처리하여 안전한 텍스트로 변환합니다.
- `url_extractor` 및 `pii_anonymizer` 도구에 대한 기능 테스트 4종을 추가하여 안정성을 검증했습니다 (`backend/tests/test_tools_api.py`). 100% 코드 커버리지를 보장합니다.
