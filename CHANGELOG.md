## [Unreleased]
- `orchestrator/*` 모델을 사용하는 chat/completion/responses/embedding·프로젝트 그래프·batch 경로는 contextual-orchestrator에 엄격한 불리언 `zdr_only=true`를 전달합니다. contextual-orchestrator는 caller가 제공한 임의의 model-group 배열과 auto-discovery 결과에서 ZDR 증거가 있는 멤버만 선별하며 후보 모델 배열을 하드코딩하지 않습니다. OpenRouter의 ZDR endpoint는 provider-neutral한 근거로 사용되어 다른 provider 모델에도 적용됩니다.
