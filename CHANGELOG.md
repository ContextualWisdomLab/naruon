## [Unreleased]
- `strip_html_markup`이 `<!--><script>…</script>-->` 같은 주석 우회 페이로드에서 남은 `-->`를 제거해, 태그형 잔여물이 평문 결과에 남지 않게 합니다.
- 메일 상세 `일정 반영`이 CalDAV If-Match 412/`etag_conflict`를 성공으로 숨기지 않고, 확정 일정 충돌을 사용자에게 알리고 `calendar_reflected.conflict_state`에 기록합니다. 충돌한 실행 항목 문구를 인용하고 `일정 충돌 조율` 배지를 켜서 원본 확정 일정이 유지됐음을 같은 화면에서 보이게 합니다.
