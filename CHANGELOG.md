## [Unreleased]
- Root README is now customer/operator first: standalone Compose boot, required
  secrets, and how a sibling calls the signed OpenAPI surface. PR-stacking,
  Phase 10, robot-review, MLX, and fixture-token dumps moved to
  `docs/development/`. Default `docker compose up` still does not need a
  sibling checkout; the optional `docker-compose.pg-llm-batch.yml` overlay
