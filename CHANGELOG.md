## [Unreleased]
- `provider_type=contextual_orchestrator`로 명시된 테넌트 provider와 batch-orchestrator 전송 경로는 모델명과 무관하게 엄격한 불리언 `zdr_only=true`를 전달하며, ZDR batch 실패 시 비-ZDR 외부 provider로 원문을 재전송하지 않습니다. contextual-orchestrator는 caller가 제공한 임의의 model-group 배열과 auto-discovery 결과에서 ZDR 증거가 있는 멤버만 선별하며 후보 모델 배열을 하드코딩하지 않습니다.
