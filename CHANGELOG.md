## [Unreleased]
- S3 통합 CI가 LocalStack의 region-qualified TLS 별칭 대신 인증서가 보장된
  `localhost.localstack.cloud` endpoint를 사용하도록 정렬했습니다. 동일한
  HTTPS·path-style·host allowlist 경계를 유지하면서 CI의 인증서 hostname
  mismatch를 제거합니다.
