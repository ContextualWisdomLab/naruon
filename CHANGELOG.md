## [Unreleased]
### Added
- `backend/api/tools.py`에 이메일 텍스트에서 연락처 정보(이메일 주소 및 전화번호)를 추출하는 `contact_info_extractor` 도구와 텍스트의 가독성 점수를 계산하는 `readability_scorer` 분석 도구를 추가했습니다.
- 추가된 도구 핸들러들에 대해 `backend/tests/test_tools_api.py`에 입력 텍스트 제한 초과 예외 상황과 정상 처리 결과를 검증하는 비동기 단위 테스트를 작성하여 테스트 커버리지를 확보했습니다.
