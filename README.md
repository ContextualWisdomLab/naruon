...
- Local HMAC sessions are suitable for local and compatibility testing. Production workspace membership should use verified OIDC/JWKS or another explicit server-side membership authority.
...
- Authorization is deny-first RBAC plus ABAC. Naruon's data-region, consent, workspace, group, source-capability, and customer-policy denies take precedence over broad role grants.
...
Naruon is proprietary software. See [LICENSE](LICENSE) for the current grant and restrictions.
