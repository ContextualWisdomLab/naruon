...
The backend authentication dependency is bearer-only at this protected head: `get_auth_context()` consumes the `Authorization` header and does not authenticate from a browser cookie. The primary cookie-bearing browser boundary is the Next.js API proxy. The backend middleware is therefore defense-in-depth for browser-derived direct traffic, not the primary session-CSRF control. That distinction is material to severity and to the decision not to reject every provenance-free API request.

Naruon also supports legitimate non-browser API clients that may carry no browser provenance headers. Treating every provenance-free request as a browser request would conflate browser provenance enforcement with service/client authentication and would change that contract without evidence.
...
Naruon's Next.js API proxy already owns a stricter browser-only boundary: merged #653 rejects state-changing proxy requests when neither Origin nor Referer is present. The backend serves bearer-authenticated API traffic directly as well as browser-derived traffic, so this PR does not copy the proxy's unconditional missing-header rejection into the backend.
...
- **Reject every state-changing request with no Origin/Referer.** Rejected because it would silently redefine provenance-free bearer/API traffic as browser traffic.
- **Describe this as a high-severity cookie-CSRF bypass.** Rejected because protected backend auth is `Authorization`-header based; the cookie-bearing Next.js proxy already has its own stricter control. The backend finding is a defense-in-depth provenance inconsistency unless a separate credential-attachment path is demonstrated.
...
5. #1361 must no longer carry this CSRF delta once this owner lane exists, so the checksum PR remains a pure checksum-owned diff.
