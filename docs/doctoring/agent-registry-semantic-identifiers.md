...
Read-only Python properties (`name`, `framework`, `entrypoint`, `description`, `capabilities`, `enabled`) remain bounded compatibility aliases for downstream package/submodule consumers. The historical mutable `raw` contract is preserved through `_LegacyRawDict`, a real `dict` subclass backed by semantic `raw_entry`. That distinction is intentional: historical consumers may use `isinstance(agent.raw, dict)`, JSON serialization, dictionary union, copying, and standard dictionary mutation methods in addition to indexed access. The compatibility dictionary keeps a legacy-shaped shallow mapping so those dict-specific operations remain valid, while every mutating operation writes through to the authoritative semantic evidence.

Reads of the six translated legacy keys address their semantic `agent_*` counterparts; assignment or deletion through `raw["name"]`, `raw["description"]`, and the other historical aliases updates/removes the corresponding semantic evidence key. `update`, `setdefault`, `pop`, `popitem`, `clear`, and in-place dictionary union follow the same write-through rule. Unrelated legacy metadata writes address the same key in `raw_entry`. A newly requested `raw` dictionary is reconstructed from current semantic evidence, so later accesses observe earlier mutations without storing generic compatibility keys in the authoritative evidence mapping. Nested list/dictionary values retain shallow aliasing, matching the historical dictionary contract.
...
The focused registry tests require the canonical semantic attributes and checked-in semantic JSON keys. They exercise the bounded legacy Python properties and verify that the old `raw` mapping keys remain readable for both semantic and historical JSON input while `raw_entry` remains free of generic compatibility keys. Reviewer-discovered regressions now have dedicated tests: one assigns and deletes translated and unrelated keys through `raw` and proves later `raw` access plus semantic `raw_entry` observe those mutations; another requires `raw` to remain an actual JSON-serializable dictionary supporting dictionary union; the mutation-method regression covers `update`, `setdefault`, `pop`, `popitem`, `clear`, and in-place union synchronization. Immutable typed fields retain their load-time values throughout.
