...
Read-only Python properties (`name`, `framework`, `entrypoint`, `description`, `capabilities`, `enabled`) remain bounded compatibility aliases for downstream package/submodule consumers. The historical mutable `raw` contract is preserved through one retained `_LegacyRawDict` per `RegisteredAgent`, paired with a `_SemanticRawDict` used by `raw_entry`. Both are real `dict` subclasses. That distinction is intentional: historical consumers may retain `agent.raw`, use `isinstance(agent.raw, dict)`, JSON serialization, dictionary union, copying, and standard dictionary mutation methods in addition to indexed access, while semantic consumers may continue to mutate the public `raw_entry` dictionary directly.

The pair is coherent and bidirectional. Reads of the six translated legacy keys address their semantic `agent_*` counterparts; assignment or deletion through `raw["name"]`, `raw["description"]`, and the other historical aliases updates/removes the corresponding semantic evidence key. `update`, `setdefault`, `pop`, `popitem`, `clear`, and in-place dictionary union follow the same write-through rule. Unrelated legacy metadata writes address the same key in `raw_entry`. Conversely, direct semantic `raw_entry` assignments, deletions, and standard dictionary mutations update the already-retained legacy dictionary using the historical aliases. Generic legacy keys are rejected from `raw_entry`, and semantic-only translated keys are rejected from `raw`, so the anti-corruption direction remains explicit.

Every `raw` property access returns the same retained legacy dictionary for that agent. This preserves historical reference identity and prevents two retained callers from observing divergent snapshots. The paired mutation methods update the authoritative and compatibility dictionaries before returning, so a later delete/pop does not encounter a stale counterpart after another reference changed the data. Nested list/dictionary values retain shallow aliasing, matching the historical dictionary contract.

The immutable typed dataclass attributes continue to represent the values captured when the registry entry was loaded. Mutating `raw` or `raw_entry` historically changes metadata/evidence rather than reassigning those frozen typed attributes, so the compatibility adapter preserves that split: the two mapping surfaces stay synchronized with each other, while `agent_name`, `agent_description`, and the other typed fields remain unchanged until a registry reload.

This keeps migration one-way at the catalog boundary and coherent in memory: legacy JSON inputs are canonicalized on ingress, organization-owned code reads semantic fields internally, and the retained compatibility dictionary translates historical raw keys at the boundary. No caller needs a flag-day migration, while new code has no reason to adopt the generic names.
...
The focused registry tests require the canonical semantic attributes and checked-in semantic JSON keys. They exercise the bounded legacy Python properties and verify that the old `raw` mapping keys remain readable for both semantic and historical JSON input while `raw_entry` remains free of generic compatibility keys. Reviewer-discovered regressions have dedicated tests: translated and unrelated raw assignments/deletions synchronize with semantic evidence; `raw` remains an actual JSON-serializable dictionary supporting dictionary union; `update`, `setdefault`, `pop`, `popitem`, `clear`, and in-place union synchronize; two retained `raw` references are the same coherent dictionary; and direct `raw_entry` assignment/deletion is reflected immediately through retained legacy references. Immutable typed fields retain their load-time values throughout.
