...
RFC 8259 §8.1 requires JSON text exchanged between systems outside a closed ecosystem to use UTF-8, while §8.2 notes that its ABNF can admit escaped surrogate fragments such as an unpaired `\uDEAD` whose downstream behavior is unpredictable. Python's parser can retain an escaped lone surrogate as a surrogate code point, and `ensure_ascii=False` can then produce a Python string that strict UTF-8 serialization cannot encode. Naruon recursively rejects surrogate code points in parsed object keys and string values before formatting so this case follows the ordinary tool-validation failure channel instead of becoming a response-serialization failure. Valid escaped surrogate pairs are decoded by Python into their corresponding Unicode scalar value and remain accepted.

- Production: `backend/api/tools.py::json_formatter_handler`, `_strict_json_object`, `_reject_nonfinite_json_constant`, `_reject_unpaired_json_surrogates`
- Regression evidence: `backend/tests/test_utility_tools_contract.py::test_json_formatter_rejects_lossy_or_nonportable_json`, `test_json_formatter_rejects_unpaired_utf16_surrogates`
- Failure-channel evidence: `backend/tests/test_utility_tools_contract.py::test_json_formatter_uses_failed_tool_channel_for_invalid_json`, `test_json_formatter_rejects_unpaired_utf16_surrogates`
...
SHA-256 and SHA-512 are members of the SHA-2 family standardized by NIST FIPS 180-4. NIST has announced its transition away from SHA-1 for cryptographic protection, and RFC 6151 states that MD5 is not acceptable where collision resistance is required. Naruon keeps MD5 and SHA-1 only as explicit compatibility/checksum choices; the product catalog classifies this feature as a utility and states that those algorithms are not for security-sensitive use. Python's `hashlib.md5` and `hashlib.sha1` calls therefore pass `usedforsecurity=False`, which makes the non-security purpose explicit to FIPS-aware runtimes instead of pretending these legacy digests are security primitives. Unknown algorithm names fail rather than falling back to another digest.
...
- **Permissive JSON normalization:** rejected because duplicate-key, non-finite-value, and invalid-surrogate handling differs across consumers and can lose source meaning or defer failure to transport serialization.
- **ASCII-escaping lone surrogates:** rejected because it preserves a JSON grammar fragment that does not identify a Unicode scalar value and would leave interoperability dependent on downstream implementation behavior.
