...
The TDD branch began with the acceptance tests committed before the resolver. The focused local harness exercised realistic RFC 5322/MIME byte messages covering CID resolution, nested related-scope boundaries, duplicate Content-ID ambiguity, repeated identical signature-like images, base64 data images, remote no-fetch behavior, malformed references, unsupported media, MIME/signature mismatches, and resource limits. Before the later aggregate-bound hardening, owned production coverage for `services/email_media_resolution.py` measured 100% statement and branch coverage; the hardening itself was again introduced test-first. Hosted exact-head repository tests and coverage remain authoritative for merge, and predecessor-head percentages are not promoted to current-head proof.
