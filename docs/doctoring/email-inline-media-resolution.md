...
| Convert `cid:` URLs to Content-ID evidence | RFC 2392 | Percent-decode the URL form, reject control/whitespace corruption in both URL and header identities, and match against normalized Content-ID header values. Duplicate matches fail closed as `review_required`. |
...
| Preserve repeated visual occurrences | Product provenance contract | Image payloads are SHA-256 content-addressed while every MIME/HTML occurrence retains its own path/span; aggregate occurrence count is independently bounded even when payloads deduplicate. |
...
The resolver enforces independent per-message bounds for raw message bytes, individual image bytes, HTML text, aggregate extracted image references across all HTML MIME parts, distinct content-addressed artifacts, and total media occurrences. These are separate because deduplication can keep artifact count small while a hostile multipart message still multiplies references or provenance records. Unsupported media, declared/signature mismatches, malformed base64, malformed Content-ID values, unresolved or ambiguous Content-ID references, and resource-limit violations are explicit result states rather than silent omissions or model guesses.
...
The TDD branch began with the acceptance tests committed before the resolver. The focused local harness exercised realistic RFC 5322/MIME byte messages covering CID resolution, nested related-scope boundaries, duplicate Content-ID ambiguity, repeated identical signature-like images, base64 data images, remote no-fetch behavior, malformed references, unsupported media, MIME/signature mismatches, and resource limits. The later digest-collision regression was also committed before its production repair and exercises both MIME traversal orders so artifact deduplication cannot upgrade a mismatched occurrence. Before the aggregate-bound hardening, owned production coverage for `services/email_media_resolution.py` measured 100% statement and branch coverage; hosted exact-head repository tests and coverage remain authoritative for merge, and predecessor-head percentages are not promoted to current-head proof.
