...
`Accept-Language` is also an HTTP list field. RFC 9110 §5.6.1.2 requires recipients to parse and ignore a reasonable number of empty list elements that can arise when field values are merged. Empty comma members therefore carry no language preference and are skipped before language-range validation; they are not reinterpreted as wildcards or product-default votes. An all-empty present field resolves exactly like an empty preference list and falls through to the product default.
...
RFC 9110 defines `Accept-Language` as weighted language ranges, defines `weight = OWS ";" OWS "q=" qvalue`, defines OWS exactly as zero or more SP/HTAB octets, explicitly states that the `q` parameter name is case-insensitive, and in §5.6.1.2 requires recipients of list-based fields to parse and ignore a reasonable number of empty list elements. It also states that field values containing CTL characters are invalid, with CR/LF/NUL specifically dangerous. RFC 4647 supplies language-range matching and RFC 5646 supplies language-tag structure. Naruon's first release intentionally reduces supported variants to eight product-language identities, so this module is a bounded product policy over those standards rather than a general-purpose BCP 47 library.
