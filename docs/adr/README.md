...
| [ADR-0005](0005-attachment-content-type-quarantine.md) | Quarantine attachments whose sniffed bytes disagree with their declared content type; address them via a new `attachment_uid` | Accepted | `ACCEPTED-NARUON-POLICY`; `reparse_pending` is consumed by `AttachmentReparseWorker` |
