...
## 4. UI Localization Catalog — v2.48 material contract
...
Eighteen ordinary-forward RED→repair sequences now define the current source policy. Durable/current invariants are:
...
18. source correctness on the repaired grandfathered-tag path does not transfer predecessor hosted evidence. Exact `502669e2...` still requires fresh full-suite/coverage/security and qualifying independent post-last-push review before policy acceptance.
...
No predecessor workflow/review receipt transfers to current #1740 exact `502669e2...`. The source repair is present, but queued/pending/current-head review evidence must still become terminal and qualifying before merge.
