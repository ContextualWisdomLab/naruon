...
**Hybrid dense + sparse search.** LIVE: `api/search.hybrid_search` combines Postgres FTS (`to_tsvector`/`ts_rank_cd`) with pgvector `cosine_distance` (`_search_score = fts_score − vector_distance`), degrading gracefully to FTS-only; scoped over `email_records`/`email_attachments` bodies. PARTIAL: `backend/services/batch_embedding_service.py` now submits latency-tolerant import/backfill embeddings to contextual-orchestrator / pg-llm-batch, preserves input order and cost attribution, and requires `zdr_only=true` for the explicit batch-orchestrator transport; ZDR work fails closed rather than using a non-ZDR external fallback. TARGET: extend search to `content_segments` and typed `project_graph_objects` (search the *meaning*) and expose rank fusion (e.g., reciprocal-rank fusion) as a `search.ranker` extension point.
...
|---|---|---|---|
| **contextual-orchestrator** | LLM cost optimizer + LB + routing hub; batch via **pg-llm-batch** | **PARTIAL / INTEGRATED** — `llm_service`, `embedding`, `batch_embedding_service`, project-graph extraction, search, and Noema forward the gateway request policy; explicitly scoped contextual-orchestrator providers and the batch transport set strict `zdr_only=true` independent of model names | Route remaining LLM call sites and productionize the released hash-locked adapter; contextual-orchestrator owns auto-discovery, provider failover, model-group selection, and the KV-backed provider credentials. |
...
- **LLM-driven, language-agnostic extraction** routed through **contextual-orchestrator** (partial: the named `orchestrator` extractor and gateway request policy are implemented; deterministic rules remain the fail-closed fallback).
...
- **Unified `/streams`** (email+chat+code+issue+audio); batch embedding routing is implemented through `batch_embedding_service`, while hybrid search over segments/entities remains planned.
...
8. **Swap deterministic extraction for LLM-based**, routed through **contextual-orchestrator**, deterministic rules retained as fallback; extend the implemented batch embedding route with the remaining `batch_embedding_jobs` product surfaces. Language-agnostic (no morphological analyzers).
