...
- **Language-agnostic (G6):** entity/relation extraction, resolution, and search work consistently across EN/KO/JA/ZH/VI via LLM extraction + multilingual embeddings + cross-lingual structured topic modeling — **no dependency on morphological analyzers** (Kiwi/Nori-style), which cause performance cliffs.
