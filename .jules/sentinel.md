...
**Prevention:** Canonicalize URL encoding only through a small bounded number of rounds, fail closed if the name still changes, reject decoded control characters such as NUL, and normalize both slash styles before extracting a basename. This covers doubly encoded and cross-platform traversal payloads without turning attacker-controlled encoding depth into unbounded work or an uncaught filesystem error.
