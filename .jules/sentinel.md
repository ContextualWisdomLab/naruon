...
## 2026-08-15 - Malformed OIDC token endpoint encoding
**Vulnerability:** A malformed deployment value for `NEXT_PUBLIC_OIDC_TOKEN_ENDPOINT`, or a token endpoint derived from a malformed configured issuer, could make `decodeURIComponent` throw while validating the trusted endpoint path.
**Learning:** The callback request supplies authorization response search data; it does not control the configured token endpoint. Invalid endpoint encoding is a deployment-configuration failure and must not be described as an attacker-controlled callback DoS.
**Prevention:** Catch URI decoding failures at trusted OIDC configuration validation and return the fixed `oidc_token_exchange_failed` HTTP 502 response without exposing the malformed endpoint.
