...
## 2025-02-12 - Memoizing inline array maps in layout components
**Learning:** Inline mapping of arrays inside JSX in large React layout components (like `AIHubLayout`) causes O(N) recalculation on every render when tabs or unrelated surface states change.
**Action:** Wrap inline JSX elements that map over potentially large arrays (e.g., lists of execution events) in a `useMemo` hook with specific dependencies to prevent rendering bottlenecks.
