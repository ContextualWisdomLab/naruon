...
## 2025-02-12 - Prevent Re-instantiation of Heavy NetworkGraph Component
**Learning:** Heavy visualization components like `NetworkGraph` that instantiate complex third-party DOM-manipulating libraries (e.g., `vis-network`) cause costly layout thrashing and performance bottlenecks when their parent layout components frequently re-render, as they rebuild the entire network un-memoized.
**Action:** Always wrap heavy child components that rely on third-party DOM manipulating libraries (like `vis-network`) in `React.memo` to prevent expensive re-renders and re-instantiation when parent components have unrelated state updates.
