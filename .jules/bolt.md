...
**Learning:** Using a dictionary (`grouped = {}` -> `has_sent_message[key] = True`) just to test boolean presence via `if key in dict` or `dict.get(key)` wastes memory overhead and degrades code semantics.
