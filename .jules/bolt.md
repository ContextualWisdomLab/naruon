...
## 2025-02-12 - Memoized EmailDetail thread item components
**Learning:** `useMemo` remains effective when its dependencies are unchanged, so an unrelated parent render does not inherently invalidate an inline memoized list. When selection-dependent values change, however, a list-level memoization boundary must recompute the whole mapped value even if most individual item props remain unchanged.
**Action:** Keep `useMemo` when the complete mapped value has stable dependencies. Extract individual list items and wrap them in `React.memo()` when unrelated parent updates leave those item props unchanged and item-level bailout avoids unnecessary reconciliation work.
