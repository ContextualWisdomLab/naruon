...
**Learning:** Using a dictionary (`has_sent_message = {}` -> `has_sent_message[key] = True`) just to test boolean presence via `if key in dict` or `dict.get(key)` wastes memory overhead and degrades code semantics.
