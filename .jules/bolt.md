...
## 2025-02-12 - Reverted "My Tasks" inline array mapping optimization
**Learning:** Extracting an inline JSX mapping (`filteredTicketTasks.map`) into a `useMemo` does not inherently prevent child component reconciliation if the items themselves are not memoized, and it may not improve performance without referential stability of the source array (`filteredTicketTasks`). Blindly applying this pattern without profiling data or proof of large cardinality adds unnecessary complexity, carries risks of stale dependencies, and establishes harmful repository "rules" that ignore context.
**Action:** Only optimize list rendering after measuring a true performance bottleneck with tools like the React Profiler. Use stable selectors and element-level memoization (`React.memo`) or virtualization when lists are genuinely large enough to block the main thread, rather than prematurely wrapping every JSX array map in `useMemo`.
