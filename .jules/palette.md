...
## 2026-09-21 - Remove redundant aria-disabled on disabled buttons
**Learning:** Applying the `aria-disabled` attribute alongside the native HTML `disabled` attribute is an accessibility anti-pattern. The native `disabled` attribute inherently communicates the unavailable state to screen readers.
**Action:** When managing disabled states for interactive elements (like buttons) for async operations or unmet conditions, only use the native `disabled` attribute along with `aria-busy` (for async actions). Never use `aria-disabled` alongside `disabled`.
