...
**Action:** When adding or auditing asynchronous auth check actions, always combine `disabled={isLoading}`, `aria-busy={isLoading}`, a decorative `Loader2` spinner with `aria-hidden="true"`, and a dynamic text label (e.g. `isLoading ? '로그인 중' : 'OIDC 로그인'`) to give immediate, unambiguous feedback.
