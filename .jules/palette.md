...
## 2026-07-02 - Ensure Design QA Mockups Are Replicated
**Learning:** Initial MVP deliveries often strip down the density of screens (like removing participant lists or attachment rails) to ship faster. However, preserving design handoff fidelity ensures a complete QA.
**Action:** When acting on design QA feedback regarding desktop density or missing mobile states, replicate the visual placeholders until dynamic real data sources are wired up, ensuring that accessibility (like `aria-hidden` on icons) is maintained.
