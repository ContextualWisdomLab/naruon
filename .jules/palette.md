...
## 2024-05-14 - Add aria-busy to loading buttons
**Learning:** Assistive technologies rely on `aria-busy` to understand when a form submission or button action is in a loading state, even if the button is visually disabled. Just setting `disabled={isLoading}` is not enough.
**Action:** Always ensure that `aria-busy={isLoading}` is set alongside `disabled={isLoading}` for action buttons that handle asynchronous tasks to communicate the specific reason for being disabled.
