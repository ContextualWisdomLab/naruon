...
## 2024-05-24 - Accessible dynamic empty states for search results
**Learning:** Empty states for search results ("맥락 검색 결과가 없습니다.") won't be announced by screen readers when results disappear.
**Action:** Ensure empty state containers have `role="status"` and `aria-live="polite"` so screen readers notify users of the change.
