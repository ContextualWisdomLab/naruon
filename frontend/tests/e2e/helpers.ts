...
const aiHubPrompts = [
  {
    id: 101,
    title: "의사결정 로그 맥락 종합",
    description: "출시 일정과 파트너 리스크를 함께 검토합니다.",
  },
  {
    id: 102,
    title: "계약 리스크 점검",
    description: "계약서, 첨부, 메일 스레드를 판단 포인트로 정리합니다.",
  },
  {
    id: 103,
    title: "후속 실행 항목",
    description: "답장, 일정, 할 일을 담당자별 실행 흐름으로 나눕니다.",
  },
];
...
const operationalSignals = {
  ...
  signals: [
    ...
    },
    {
      ...
      detail:
        "Provider sync lag will be emitted by source-backed connector jobs.",
      provider_write_executed: false,
    },
    {
  ...
  attachment_parse_breakdown: [
    {
      ...
      object_count: 2,
      evidence_source:
        "email_attachments.content_type, email_attachments.parse_content_type, email_attachments.parse_status, email_attachments.parser_key",
      provider_write_executed: false,
    },
    {
...
export async function mockDashboardApi(
  page: Page,
  onApiRequest?: (path: string, request: Request) => void,
) {
  await page.route("**/api/**", async (route) => {
    ...
    if (
      path === "/api/observability/operational-signals" &&
      request.method() === "GET"
    ) {
    ...
    if (path.startsWith("/api/llm-providers/") && request.method() === "PUT") {
      ...
      const provider =
        llmProviders.find((candidate) => candidate.id === providerId) ??
        llmProviders[0];
      await fulfillJson(route, {
        ...provider,
        ...body,
    ...
    if (path === "/api/search" && request.method() === "POST") {
      ...
      await fulfillJson(route, {
        results: [email, mobileAttachmentResult, mobilePeopleResult],
      });
      return;
    }
    ...
    if (path === "/api/llm/summarize") {
      await fulfillJson(route, {
        summary:
          "출시 일정, 마케팅 계획, 파트너 미팅을 하나의 실행 흐름으로 정리해야 합니다.",
    ...
    if (path === "/api/llm/draft") {
      await fulfillJson(route, {
        draft: "검토 후 일정과 우선순위를 정리해 공유드리겠습니다.",
      });
      return;
    }
    ...
    if (
      path === "/api/calendar/writeback-sources" &&
      request.method() === "GET"
    ) {
      await fulfillJson(route, [
        {
    ...
    if (
      path === "/api/calendar/writeback-intent" &&
      request.method() === "POST"
    ) {
      await fulfillJson(route, {
    ...
    if (
      path === "/api/webdav/writeback-intent" &&
      request.method() === "POST"
    ) {
      await fulfillJson(route, {
        intent: "writeback",
    ...
    if (
      path === "/api/emails/unique-thread-intent" &&
      request.method() === "POST"
    ) {
      await fulfillJson(route, {
        status: "intent_ready",
    ...
    if (
      path === "/api/webdav/knowledge-materialization-intent" &&
      request.method() === "POST"
    ) {
      await fulfillJson(route, {
    ...
    if (
      path === "/api/tasks/reply-sla-escalations" &&
      request.method() === "POST"
    ) {
      await fulfillJson(route, {
        evaluated: 2,
