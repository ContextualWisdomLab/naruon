...
**Action:** When a disabled button needs a tooltip to explain *why* it is disabled, wrap the button in an accessible container (e.g., `span` or `div` with `tabIndex={0}`) and place the `title` attribute on the wrapper. Also, ensure the button uses `pointer-events-none` so the wrapper can properly catch the hover events.
