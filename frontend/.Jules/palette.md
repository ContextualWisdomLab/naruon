...
## 2026-06-08 - Accessible Tooltips on Disabled Buttons (Update)
**Learning:** Raw HTML `<button>` tags with Tailwind classes for standard interaction patterns are often inconsistent and fail to provide robust accessibility states out-of-the-box compared to design system components. We found that the `CalendarSidebarRight` component was using raw buttons for its primary actions, leading to non-standard focus rings and hardcoded corner radii.
**Action:** Always replace raw HTML `<button>` tags with the `@/components/ui/button` `Button` component, matching the intended design variants (`outline`, `default`, `link`) to enforce design system consistency, ensure accessible focus states (`focus-visible:ring-2`), and guarantee correct interactive feedback across the app.
