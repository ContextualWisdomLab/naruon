    ...
    <aside className="w-[340px] shrink-0 flex-col overflow-y-auto border-l border-border bg-card p-5 hidden xl:flex">
      ...
      <div className="mt-6 space-y-5">
        ...
        <div className="flex gap-3 items-center">
          ...
          <button type="button" aria-disabled={!selectedDetailEvent?.location} onClick={(e) => { if (!selectedDetailEvent?.location) { e.preventDefault(); return; } }} aria-label={`${selectedDetailEvent?.location ?? '장소'} 위치 보기`} className="text-xs text-primary font-semibold ml-auto hover:underline rounded-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/40 aria-disabled:opacity-50 aria-disabled:cursor-not-allowed aria-disabled:hover:no-underline">위치 보기</button>
        </div>
      ...
      <div className="mt-8 flex gap-3">
        <button type="button" aria-disabled={!selectedDetailEvent} onClick={(e) => { if (!selectedDetailEvent) { e.preventDefault(); return; } }} aria-label={selectedDetailEvent ? `${selectedDetailEvent.title} 일정 삭제` : '일정 삭제'} className="flex-1 rounded-lg border border-border bg-background py-2 text-sm font-bold shadow-sm hover:bg-secondary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/40 aria-disabled:opacity-50 aria-disabled:cursor-not-allowed">삭제</button>
        <button type="button" aria-disabled={!selectedDetailEvent} onClick={(e) => { if (!selectedDetailEvent) { e.preventDefault(); return; } }} aria-label={selectedDetailEvent ? `${selectedDetailEvent.title} 일정 복사` : '일정 복사'} className="flex-1 rounded-lg border border-border bg-background py-2 text-sm font-bold shadow-sm hover:bg-secondary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/40 aria-disabled:opacity-50 aria-disabled:cursor-not-allowed">복사</button>
        <button type="button" aria-disabled={!selectedDetailEvent} onClick={(e) => { if (!selectedDetailEvent) { e.preventDefault(); return; } }} aria-label={selectedDetailEvent ? `${selectedDetailEvent.title} 일정 수정` : '일정 수정'} className="flex-1 rounded-lg bg-primary py-2 text-sm font-bold text-primary-foreground shadow-sm hover:bg-primary/90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/40 aria-disabled:opacity-50 aria-disabled:cursor-not-allowed">수정</button>
      </div>
    </aside>
  );
}
