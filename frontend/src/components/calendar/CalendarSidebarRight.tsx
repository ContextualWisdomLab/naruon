    ...
    <aside className="w-[340px] shrink-0 flex-col overflow-y-auto border-l border-border bg-card p-5 hidden xl:flex">
      ...
      <div className="mt-6 space-y-5">
        ...
        <div className="flex gap-3 items-center">
          ...
          <Button type="button" variant="link" size="sm" disabled={!selectedDetailEvent?.location} aria-label={`${selectedDetailEvent?.location ?? '장소'} 위치 보기`} className="h-auto p-0 text-xs ml-auto">위치 보기</Button>
        </div>
      ...
      <div className="mt-8 flex gap-3">
        <Button type="button" variant="outline" disabled={!selectedDetailEvent} aria-label={selectedDetailEvent ? `${selectedDetailEvent.title} 일정 삭제` : '일정 삭제'} className="flex-1 shadow-sm">삭제</Button>
        <Button type="button" variant="outline" disabled={!selectedDetailEvent} aria-label={selectedDetailEvent ? `${selectedDetailEvent.title} 일정 복사` : '일정 복사'} className="flex-1 shadow-sm">복사</Button>
        <Button type="button" variant="default" disabled={!selectedDetailEvent} aria-label={selectedDetailEvent ? `${selectedDetailEvent.title} 일정 수정` : '일정 수정'} className="flex-1 shadow-sm">수정</Button>
      </div>
    </aside>
  );
}
