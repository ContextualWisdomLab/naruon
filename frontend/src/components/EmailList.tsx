...
async function fetchFolderEmails(folder: MailFolder) {
  ...
  const request = apiClient
    .get<{ emails: EmailItem[] }>(folderEndpoint(folder))
...
const EmailListItemComponent = memo(function EmailListItemComponent({
  ...
  return (
    ...
    >
      <span
        className={`absolute inset-y-3 left-0 w-1 rounded-r-full ${selected ? "bg-primary" : "bg-transparent group-hover:bg-primary/50"}`}
        aria-hidden="true"
      />
      <div className="flex w-full flex-col gap-1">
        <div className="flex items-center">
          <div className="flex items-center gap-2">
            <Avatar className="h-8 w-8 border border-primary/10 bg-primary/10 text-primary">
              <AvatarFallback>
                {safeSender ? safeSender.charAt(0).toUpperCase() : "U"}
              </AvatarFallback>
            </Avatar>
            <div className="max-w-[140px] truncate font-bold text-foreground">
              {safeSender}
            </div>
          </div>
        ...
        <div className="flex items-center gap-2 w-full">
          <div className="truncate text-sm font-bold flex-1 text-foreground">
            {safeSubject}
          </div>
          {email.reply_count && email.reply_count > 1 && (
            <Badge
              variant="secondary"
              className="h-5 whitespace-nowrap border border-primary/10 bg-primary/10 px-2 py-0 text-[10px] leading-none text-primary flex items-center gap-1"
            >
      ...
      {(email.unread ||
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {(email.unread || email.is_read === false) && (
            <Badge
              variant="default"
          ...
          {email.is_self_sent && (
            <Badge
              variant="secondary"
              className="border-purple-500/20 bg-purple-500/10 text-[10px] text-purple-700"
            >
              지식 정리
            </Badge>
          )}
          {email.requires_reply && (
            <Badge
              variant="outline"
              className="border-primary/30 text-[10px] text-primary bg-primary/5"
            >
              응답 대기 중
            </Badge>
          )}
          {email.schedule_conflict && (
            <Badge
              variant="outline"
              className="border-emerald-500/30 text-[10px] text-emerald-700 bg-emerald-500/5"
            >
              일정 충돌 조율
            </Badge>
          )}
...
export function EmailList({
    ...
    async (query = "") => {
      ...
      try {
        if (query.trim() === "") {
          ...
          setIsSearching(true);
          const data = await apiClient.post<{ results: EmailItem[] }>(
            "/api/search",
            { query },
          );
  ...
  const folderCopy =
    folder === "sent"
      ? {
          title: "보낸 메일",
          description:
            "답변 대기, 회신 완료, 나에게 보낸 지식 정리 후보를 추적합니다.",
          primaryBadge: "답변 추적",
          ...
          emptyBody:
            "SMTP/IMAP 동기화 후 보낸 스레드와 답변 대기 상태가 표시됩니다.",
        }
      : {
          title: "받은편지함",
  ...
  // ⚡ Bolt: Memoize email list mapping to prevent unnecessary re-renders when other state changes
  // 🎯 Why: Re-mapping over a large array of emails is computationally expensive and causes DOM reconciliation overhead.
          ...
          <div>
            ...
            <div className="mt-3 rounded-2xl border border-primary/15 bg-background/80 px-3 py-2 text-xs leading-5 shadow-inner shadow-slate-950/[0.02]">
              <span className="font-bold text-primary">
                {folderCopy.focusLabel}
              </span>
              <span className="mx-2 text-muted-foreground">·</span>
              <span className="font-semibold text-foreground">
                {folderCopy.focusText}
              </span>
            </div>
          </div>
