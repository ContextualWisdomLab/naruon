...
describe("NetworkGraph", () => {
  ...
  it("preserves Map insertion order when limiting relationship and node options (boundary checks)", async () => {
    ...
    const relSelect = mountedContainer.querySelector("select[aria-label='관계 선택']") as HTMLSelectElement;
    const relOptions = Array.from(relSelect.querySelectorAll("option")).filter(o => o.value !== "");
    expect(relOptions.length).toBe(5);

    expect(relOptions[0].value).toBe("edge-0");
    expect(relOptions[4].value).toBe("edge-4");

    const nodeSelect = mountedContainer.querySelector("select[aria-label='노드 선택']") as HTMLSelectElement;
    const nodeOptions = Array.from(nodeSelect.querySelectorAll("option")).filter(o => o.value !== "");
    expect(nodeOptions.length).toBe(8);

    expect(nodeOptions[0].value).toBe("node-0");
    expect(nodeOptions[7].value).toBe("node-7");
