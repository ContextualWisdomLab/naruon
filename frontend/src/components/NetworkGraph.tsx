...
export default function NetworkGraph() {
  ...
  const relationshipOptions = useMemo(() => {
    const options = [];
    let index = 0;
    // O(1) memory optimization: use a bounded for...of loop instead of Array.from(edgeMap.values()).slice(0, 5)
  ...
  const nodeOptions = useMemo(() => {
    const options = [];
    let index = 0;
    // O(1) memory optimization: use a bounded for...of loop instead of Array.from(nodeInstanceMap.values()).slice(0, 8)
