...
describe("EmailDetail", () => {
  ...
  it("renders rich email metadata without exposing unavailable actions", async () => {
    ...
    expect(meetingProposal?.textContent).toContain("일정 수락 기능은 준비 중입니다.");

    const acceptButton = Array.from(container.querySelectorAll("button")).find((button) => button.textContent?.includes("일정 수락"));
    expect(acceptButton).toBeInstanceOf(HTMLButtonElement);
    expect(acceptButton?.disabled).toBe(true);
    expect(acceptButton?.type).toBe("button");
    expect(acceptButton?.getAttribute("aria-describedby")).toBe("meeting-accept-unavailable-22");
    expect(acceptButton?.hasAttribute("title")).toBe(false);
  });
