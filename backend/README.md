...
`core.telemetry.setup_telemetry(app, config)` before the first request, or an
operator can set `CWL_TELEMETRY_RECEIVER` to opt in at app startup. The latter
reads a scoped token from `CWL_TELEMETRY_TOKEN_FILE`, optionally trusts
`CWL_TELEMETRY_CA_FILE`, and uses `CWL_SOURCE_REVISION` for exact source identity.
The Docker build sets that revision from `OCI_IMAGE_REVISION`. The SDK validates
HTTPS, identity, and credentials before creating exporters. Missing or invalid
opt-in settings leave product requests available and telemetry inactive; the
operational-signals API reports the actual runtime state. The backend no longer
interprets `ENABLE_OTEL`, `OTEL_EXPORTER_OTLP_ENDPOINT`, or
`OTEL_EXPORTER_OTLP_INSECURE`. The draft `telemetry` extra pins the shared SDK
commit for integration testing. The hashed Docker dependency set still needs a
reviewed released wheel and checksum before the image can use this opt-in path.
