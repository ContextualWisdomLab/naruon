...
def _exception_location_key(exc: BaseException) -> str:
    """Return an internal location key used only as fingerprint input."""
...
def _exception_fingerprint(exc: BaseException) -> str:
    """Return a stable non-secret correlation fingerprint for an exception site."""
...
def redacted_exception_info(
    ...
    fingerprint from the exception type and deepest module/function identity; it
    never attaches that traceback to the log record and never renders
    ``str(exc)`` or ``repr(exc)``.
    """
    safe_message = (
