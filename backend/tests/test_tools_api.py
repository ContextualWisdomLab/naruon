...
async def test_url_encoder_tool_success():
    ...
    assert result["encoded_url"] == "hello%20world%21"
...
async def test_url_decoder_tool_success():
    with TestClient(app) as client:
        response = client.post(
            ...
            headers={"Authorization": f"Bearer {_signed_session_token()}"},
            json={"parameters": {"encoded_url": "hello%20world%21"}},
        )
    assert response.status_code == 200
    data = response.json()
    assert data["status"] == "success"
    result = data["result"]
    assert "decoded_url" in result
    assert result["decoded_url"] == "hello world!"
...
async def test_json_formatter_tool_success():
    with TestClient(app) as client:
        response = client.post(
            ...
            headers={"Authorization": f"Bearer {_signed_session_token()}"},
            json={"parameters": {"json_string": '{"a":1,"b":"test"}'}},
        )
    assert response.status_code == 200
    data = response.json()
    assert data["status"] == "success"
    ...
    assert '{\n  "a": 1,\n  "b": "test"\n}' in result["formatted_json"]

@pytest.mark.asyncio
async def test_json_formatter_tool_invalid_json():
    with TestClient(app) as client:
        response = client.post(
            "/api/tools/json_formatter/execute",
            headers={"Authorization": f"Bearer {_signed_session_token()}"},
            json={"parameters": {"json_string": '{a:1}'}},
        )
    assert response.status_code == 200
    data = response.json()
    assert data["status"] == "success"
    result = data["result"]
    assert "formatted_json" in result
    assert "Invalid JSON" in result["formatted_json"]
...
async def test_hash_generator_tool_sha512():
    ...
    result = data["result"]
    assert "hash" in result
    assert result["hash"] == "9b71d224bd62f3785d96d46ad3ea3d73319bfbc2890caadae2dff72519673ca72323c3d99ba5c11d7c7acc6e14b8c5da0c4663475c2e5c3adef46f73bcdec043"

@pytest.mark.asyncio
async def test_hash_generator_tool_unknown_algo():
    with TestClient(app) as client:
        response = client.post(
            "/api/tools/hash_generator/execute",
            headers={"Authorization": f"Bearer {_signed_session_token()}"},
