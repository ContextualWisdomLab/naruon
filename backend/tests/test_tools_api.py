...
@pytest.mark.asyncio
async def test_email_translator_korean():
    with TestClient(app) as client:
        response = client.post(
            "/api/tools/email_translator/execute",
            headers={"Authorization": f"Bearer {_signed_session_token()}"},
...
@pytest.mark.asyncio
async def test_email_translator_unknown():
    with TestClient(app) as client:
        response = client.post(
            "/api/tools/email_translator/execute",
            headers={"Authorization": f"Bearer {_signed_session_token()}"},
...
async def test_sentiment_analyzer_positive():
    with TestClient(app) as client:
        response = client.post(
            "/api/tools/sentiment_analyzer/execute",
            headers={"Authorization": f"Bearer {_signed_session_token()}"},
            json={"parameters": {"text": "Thank you very much. This is great!"}},
        )
    assert response.status_code == 200
    data = response.json()
    assert data["status"] == "success"
...
async def test_sentiment_analyzer_neutral():
    with TestClient(app) as client:
        response = client.post(
            "/api/tools/sentiment_analyzer/execute",
            headers={"Authorization": f"Bearer {_signed_session_token()}"},
            json={"parameters": {"text": "Just checking in."}},
        )
    assert response.status_code == 200
    data = response.json()
    assert data["status"] == "success"
...
async def test_hash_generator():
    ...
    assert response.status_code == 200
    data = response.json()
    assert data["result"]["hash_value"] == "5d41402abc4b2a76b9719d911017c592"
    ...
    assert response.status_code == 200
    data = response.json()
    assert data["result"]["hash_value"] == "aaf4c61ddcc5e8a2dabede0f3b482cd9aea9434d"
    ...
    assert response.status_code == 200
    data = response.json()
    assert data["result"]["hash_value"] == "9b71d224bd62f3785d96d46ad3ea3d73319bfbc2890caadae2dff72519673ca72323c3d99ba5c11d7c7acc6e14b8c5da0c4663475c2e5c3adef46f73bcdec043"
    ...
    assert response.status_code == 200
    data = response.json()
    assert data["result"]["hash_value"] == "2cf24dba5fb0a30e26e83b2ac5b9e29e1b161e5c1fa7425e73043362938b9824"


@pytest.mark.asyncio
async def test_json_formatter_valid_indent():
    with TestClient(app) as client:
        response = client.post(
            "/api/tools/json_formatter/execute",
            headers={"Authorization": f"Bearer {_signed_session_token()}"},
            json={"parameters": {"json_string": "{\"a\":1,\"b\":2}", "minify": False}},
        )
    assert response.status_code == 200
    data = response.json()
...
@pytest.mark.asyncio
async def test_json_formatter_valid_minify():
    with TestClient(app) as client:
        response = client.post(
            "/api/tools/json_formatter/execute",
            headers={"Authorization": f"Bearer {_signed_session_token()}"},
            json={"parameters": {"json_string": "{\n\"a\"  : \n 1,\n\"b\":2}", "minify": True}},
        )
    assert response.status_code == 200
    data = response.json()
    assert data["result"]["is_valid"] is True
    assert "{\"a\":1,\"b\":2}" in data["result"]["formatted_json"]
...
async def test_json_formatter_invalid():
    with TestClient(app) as client:
        response = client.post(
            "/api/tools/json_formatter/execute",
            headers={"Authorization": f"Bearer {_signed_session_token()}"},
            json={"parameters": {"json_string": "{\"a\":1", "minify": False}},
        )
    assert response.status_code == 200
    data = response.json()
...
@pytest.mark.asyncio
async def test_date_calculator_iso():
    with TestClient(app) as client:
        response = client.post(
            "/api/tools/date_calculator/execute",
            headers={"Authorization": f"Bearer {_signed_session_token()}"},
            json={"parameters": {"base_date": "2023-10-01T12:00:00Z", "days": 5}},
        )
    assert response.status_code == 200
    data = response.json()
    assert data["result"]["calculated_date"] == "2023-10-06"


@pytest.mark.asyncio
async def test_date_calculator_ymd():
    with TestClient(app) as client:
        response = client.post(
            "/api/tools/date_calculator/execute",
            headers={"Authorization": f"Bearer {_signed_session_token()}"},
            json={"parameters": {"base_date": "2023-10-01", "days": -2}},
        )
    assert response.status_code == 200
    data = response.json()
    assert data["result"]["calculated_date"] == "2023-09-29"


@pytest.mark.asyncio
async def test_date_calculator_invalid():
    with TestClient(app) as client:
        response = client.post(
            "/api/tools/date_calculator/execute",
            headers={"Authorization": f"Bearer {_signed_session_token()}"},
            json={"parameters": {"base_date": "not-a-date", "days": 0}},
        )
    assert response.status_code == 200
    data = response.json()
    assert data["status"] == "failed"
