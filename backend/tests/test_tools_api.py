...
async def test_url_decoder_tool_success():
    ...
    result = await url_decoder_handler({"encoded_url": "Hello%20World%21"})
...
async def test_hash_generator_tool_success():
    ...
    result = await hash_generator_handler({"text": "test", "algorithm": "sha256"})
    expected = hashlib.sha256(b"test").hexdigest()
    assert result["hash"] == expected
    assert result["algorithm"] == "sha256"

    # default algorithm test
    result2 = await hash_generator_handler({"text": "test"})
    assert result2["hash"] == expected
    assert result2["algorithm"] == "sha256"

@pytest.mark.asyncio
async def test_hash_generator_tool_unsupported_algorithm():
    from api.tools import hash_generator_handler

    with pytest.raises(ValueError, match="Unsupported hash algorithm: unsupported_algo"):
        await hash_generator_handler({"text": "test", "algorithm": "unsupported_algo"})
