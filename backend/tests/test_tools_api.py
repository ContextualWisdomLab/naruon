...
def test_execute_sentiment_analyzer_positive():
    with TestClient(app) as client:
        response = client.post(
            "/api/tools/sentiment_analyzer/execute",
            headers={"Authorization": f"Bearer {_signed_session_token()}"},
            json={"parameters": {"text": "Thank you, this is great!"}},
        )
    assert response.status_code == 200
    data = response.json()
    assert data["status"] == "success"
...
def test_execute_sentiment_analyzer_neutral():
    with TestClient(app) as client:
        response = client.post(
            "/api/tools/sentiment_analyzer/execute",
            headers={"Authorization": f"Bearer {_signed_session_token()}"},
            json={"parameters": {"text": "I am eating an apple."}},
        )
    assert response.status_code == 200
    data = response.json()
    assert data["status"] == "success"
...
def test_execute_pii_redactor():
    with TestClient(app) as client:
        response = client.post(
            ...
            json={
                "parameters": {
                    "text": "My email is test@example.com and phone is 010-1234-5678."
                }
            },
        )
    assert response.status_code == 200
    data = response.json()
    assert data["status"] == "success"
    assert "[REDACTED_EMAIL]" in data["result"]["redacted_text"]
    assert "[REDACTED_PHONE]" in data["result"]["redacted_text"]
    assert "test@example.com" not in data["result"]["redacted_text"]
    assert "010-1234-5678" not in data["result"]["redacted_text"]


def test_execute_reading_time_estimator():
    with TestClient(app) as client:
        response = client.post(
            "/api/tools/reading_time_estimator/execute",
            headers={"Authorization": f"Bearer {_signed_session_token()}"},
            json={
                "parameters": {
                    "text": "This is a simple text with few words."
                }
            },
        )
    assert response.status_code == 200
    data = response.json()
    assert data["status"] == "success"
    assert data["result"]["word_count"] == 8
    assert data["result"]["estimated_minutes"] == 1
    ...
    assert data_zero["result"]["word_count"] == 0
    assert data_zero["result"]["estimated_minutes"] == 0
