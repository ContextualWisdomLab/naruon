...
@pytest.mark.asyncio
async def test_json_formatter_tool_rejects_nan():
    with TestClient(app) as client:
        response = client.post(
            "/api/tools/json_formatter/execute",
            headers={"Authorization": f"Bearer {_signed_session_token()}"},
    ...
    data = response.json()
    assert data["status"] == "failed"
    assert "Non-standard JSON constant: NaN" in data["message"]
    assert data.get("result") is None


@pytest.mark.asyncio
async def test_json_formatter_tool_preserves_unicode():
    with TestClient(app) as client:
        response = client.post(
            "/api/tools/json_formatter/execute",
            headers={"Authorization": f"Bearer {_signed_session_token()}"},
            json={"parameters": {"json_string": '{"key":"한글"}'}},
        )
    assert response.status_code == 200
    data = response.json()
    assert data["status"] == "success"
