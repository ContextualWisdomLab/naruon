...
def test_json_formatter_api_preserves_reserved_looking_user_object() -> None:
    """User objects must never collide with an internal numeric-render sentinel."""
    data = _execute_formatter(
        '{"wrapper":{"__raw_float__":"1.25"},"value":1.25}'
    )

    assert data["status"] == "success"
    result = data["result"]
    assert isinstance(result, dict)
    parsed = json.loads(str(result["formatted_json"]), parse_float=Decimal)
    assert parsed == {
        "wrapper": {"__raw_float__": "1.25"},
        "value": Decimal("1.25"),
    }
