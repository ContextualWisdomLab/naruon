...
@pytest.mark.asyncio
async def test_imap_socket_pins_validated_public_address_across_dns_rebinding(
    monkeypatch,
):
    monkeypatch.setattr(
    ...
    def validation_resolution(*args, **kwargs):
        getaddrinfo_calls.append((args, kwargs))
        return [(socket.AF_INET, socket.SOCK_STREAM, 6, "", ("8.8.8.8", 993))]

    monkeypatch.setattr(socket, "getaddrinfo", validation_resolution)
    destination = validate_imap_destination("imap.example.com", 993)

    def fail_second_resolution(*args, **kwargs):
        raise AssertionError("pinned IMAP connection must not resolve DNS again")

    monkeypatch.setattr(socket, "getaddrinfo", fail_second_resolution)
    connected_sockaddrs = []

    async def connect_socket(sock, sockaddr):
        connected_sockaddrs.append(sockaddr)

    monkeypatch.setattr(asyncio.get_running_loop(), "sock_connect", connect_socket)
    imap_socket = await email_client.connect_validated_imap_socket(destination)
    try:
        assert connected_sockaddrs == [("8.8.8.8", 993)]
    finally:
        imap_socket.close()
