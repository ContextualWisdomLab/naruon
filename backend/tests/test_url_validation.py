...
def test_validate_same_or_subdomain_host_rejects_suffix_confusion():
    for valid_host in ("issuer.example.com", "jwks.issuer.example.com", "a.b.c.issuer.example.com"):
        validate_same_or_subdomain_host(
            "OIDC_JWKS_URL", valid_host, "OIDC_ISSUER_URL", "issuer.example.com"
        )

    for invalid_host in ("other.com", "fakeissuer.example.com", "issuer.example.com.attacker.com", "notexample.com"):
        with pytest.raises(ValueError, match="OIDC_JWKS_URL host must match or be a subdomain of OIDC_ISSUER_URL host"):
            validate_same_or_subdomain_host(
