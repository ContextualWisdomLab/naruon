...
def test_validate_same_or_subdomain_host():
    # Exact match
    validate_same_or_subdomain_host(
        "setting", "example.com", "base_setting", "example.com"
    )

    # Subdomain match
    validate_same_or_subdomain_host(
        "setting", "sub.example.com", "base_setting", "example.com"
    )

    # Multi-level subdomain match
    validate_same_or_subdomain_host(
        "setting", "a.b.c.example.com", "base_setting", "example.com"
    )

    # Mismatch
    with pytest.raises(
        ValueError,
        match="setting host must match or be a subdomain of base_setting host",
    ):
        validate_same_or_subdomain_host(
            "setting", "other.com", "base_setting", "example.com"
        )

    # Suffix match trap (should fail)
    with pytest.raises(
        ValueError,
        match="setting host must match or be a subdomain of base_setting host",
    ):
        validate_same_or_subdomain_host(
            "setting", "notexample.com", "base_setting", "example.com"
        )
