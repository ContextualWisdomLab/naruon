...
registry.register(
    ToolInfo(
        ...
        description="일반 텍스트를 URL 인코딩 형식으로 변환합니다.",
        category="유틸리티",
        parameters={"text": "string"},
    ),
    url_encoder_handler,
...
async def hash_generator_handler(params: Dict[str, Any]) -> Dict[str, str]:
    ...
    allowed = {'md5', 'sha1', 'sha224', 'sha256', 'sha384', 'sha512', 'blake2b', 'blake2s', 'sha3_224', 'sha3_256', 'sha3_384', 'sha3_512'}
...
registry.register(
    ToolInfo(
        ...
        description="주어진 문자열에 대해 지정된 알고리즘(예: md5, sha1, sha256)을 사용하여 해시값을 생성합니다.",
        category="유틸리티",
