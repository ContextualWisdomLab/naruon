...
class EmailImportBatchContext:
    """Scope needed to route bulk import embeddings through pg-llm-batch.

    Carried alongside the embedding provider so ``_generate_import_embeddings``
    can resolve per-tenant batch settings (Fernet DB) and record batch jobs.
    """
...
async def _generate_import_embeddings(
    ...
    *,
    ...
    if batch_context is not None and texts:
        # Bulk import embeddings are latency-tolerant: try the pg-llm-batch
        # engine first. A None result means batch is unconfigured/unavailable, so
        # we transparently fall through to the existing per-request path below.
